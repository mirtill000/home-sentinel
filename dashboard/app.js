...
function renderWifiPage(container) {
  ...
  container.innerHTML = `
    ...
    ${showAll && state.handshakeRows.length ? `<div class="page-section card" id="wifi-handshakes-mount"></div>` : ""}
  ...
  if (showAll) {
    ...
    if (state.handshakeRows.length) renderHandshakeCapturesTable(document.getElementById("wifi-handshakes-mount"));
  ...
  container.innerHTML = `
    ...
    <div class="table-scroll">
      <table class="data-table">
        ...
        </tr>`).join("") || '<tr><td colspan="6"><p class="empty-state">No handshake captured yet.</p></td></tr>'}</tbody>
      </table>
    </div>`;
}
